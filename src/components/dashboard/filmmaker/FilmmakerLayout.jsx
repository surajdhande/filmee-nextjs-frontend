...
export default function FilmmakerLayout({ children }) {
  ...
  const [user] = useState(() => {
    if (typeof window !== "undefined") {
      ...
      return storedUser ? JSON.parse(storedUser) : { full_name: "Filmmaker" };
    }
    return { full_name: "Filmmaker" };
  });
  ...
  const profileName = user
    ? `${user.first_name || ""} ${user.last_name || ""}`.trim() || user.full_name || "Filmmaker"
    : "Filmmaker";
        ...
        <header className="w-full border-b border-[#2A2A2A] bg-[#0E0E0E]">
          <div className="flex items-center justify-between px-6 py-4 gap-2">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <button
                onClick={() => router.push("/dashboard/filmmaker/profile")}
