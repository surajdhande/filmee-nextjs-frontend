...
/* ─────────────────────────────────────────
   ...
   ───────────────────────────────────────── */
const MOCK_PROFILE = {
  full_name: "Mervin Personal",
  role: "Talent",
  rating: 4.9,
  reviews: 127,
  location: "Los Angeles, CA",
  experience: "15+ years experience",
  email: "mervin.personal@gmail.com",
  phone: "+1 (555) 123-4567",
  website: "www.example.com",
  bio: "Professional cinematographer specializing in thriller and sci-fi genres.",
  ...
  achievements: [
    ...
    "Over $10M in successful film investments"
  ]
};
      ...
      <div className="flex flex-col md:flex-row gap-5">
        ...
        <div className="flex flex-col items-center justify-center gap-3 rounded-2xl bg-[#171717] border border-zinc-800 p-8 w-full md:w-56 shrink-0">
          ...
          <div className="flex items-center gap-1.5 text-xs text-zinc-400">
            ...
            <span>{isEditing ? `${editFormData.years_of_experience}+ years experience` : profile.experience}</span>
          </div>
        </div>
        ...
        <div className="flex-1 rounded-2xl bg-[#171717] border border-zinc-800 p-6">
          ...
          <div className="mt-5">
            ...
            <div className="flex flex-wrap gap-2 mb-3">
              {(isEditing ? editFormData.skills : profile.skills).map((skill) => (
                <span
                  key={skill}
...
export default function TalentProfileSettings() {
  ...
  const loadData = async () => {
    try {
      ...
      try {
        ...
        console.warn("Backend API getMyProfile failed, falling back to mock data:", apiErr);
      }

      if (prof) {
        const full = prof.full_name || `${prof.first_name} ${prof.last_name}`.trim() || "Mervin Personal";
        setProfile({
          full_name: full,
          first_name: prof.first_name,
          last_name: prof.last_name,
          role: prof.user_role || "Talent",
          rating: 4.9, 
          reviews: 127, 
          ...
          email: prof.email || "mervin.personal@gmail.com",
          phone: prof.phone_number || "+1 (555) 123-4567",
          website: prof.website_portfolio_url || "www.example.com",
          bio: prof.bio || "Professional cinematographer specializing in thriller and sci-fi genres.",
          skills: prof.skills && prof.skills.length > 0 ? prof.skills : ["Cinematography", "Lighting", "Color Grading"],
          avatar: prof.profile_image_url || "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?auto=format&fit=crop&w=256&q=80",
  ...
  const handleStartEdit = () => {
    ...
    setEditFormData({
      ...
      bio: profile.bio,
      years_of_experience: profile.years_of_experience || 15,
  ...
  const handleAddAchievement = () => {
    if (achievementInput.trim() && !editFormData.achievements.includes(achievementInput.trim())) {
      setEditFormData((prev) => ({
        ...prev,
      ...
      <header className="w-full shrink-0 border-b border-zinc-800 bg-[#111111] h-16 flex items-center px-4 sm:px-8 justify-between z-10">
        <div className="flex items-center gap-3">
          <div className="flex flex-col leading-none">
            <span className="text-[8px] tracking-[0.3em] font-extrabold text-zinc-400">THE</span>
            <span className="text-sm tracking-[0.1em] font-black text-white">FILMEE</span>
            <span className="text-[9px] tracking-[0.2em] font-bold text-red-500">GRID</span>
          </div>
          <div className="border-l border-zinc-800 pl-3 ml-1 hidden xs:block">
            <h1 className="text-sm font-black text-white tracking-tight">Talent Dashboard</h1>
            <p className="text-[10px] text-zinc-500">Profile &amp; Settings</p>
          </div>
        </div>
        ...
        <div className="mx-auto max-w-5xl px-4 sm:px-8 py-6 sm:py-8">
          ...
          {error && (
            <div className="mb-6 p-4 bg-red-950/30 border border-red-900 rounded-2xl text-xs text-red-400 font-bold uppercase tracking-wider flex items-center justify-between">
              <span>{error}</span>
              <button onClick={() => setError(null)} className="text-red-500 hover:text-white font-extrabold text-sm ml-4">&times;</button>
            </div>
          )}
