...
export default function InvestorsStats() {
  return (
    <section className="relative overflow-hidden bg-black px-4 pt-6 pb-4 sm:px-8 sm:pb-2 lg:px-16">
        ...
        <div className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4 lg:gap-6">
          {stats.map((stat) => (
            <div key={stat.label} className="relative">
              ...
              />
              <div
                className="flex flex-col items-center rounded-2xl border border-zinc-900 px-3 py-5 sm:px-6 sm:py-8 text-center"
                style={{ backgroundColor: "rgb(16, 24, 40)" }}
              >
                <stat.icon
                  size={24}
                  className="mb-3 sm:mb-4 text-red-500"
                />
                <span className="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-red-500">
                  {stat.value}
                </span>
                <span className="mt-1 text-xs sm:text-sm text-zinc-400">{stat.label}</span>
              </div>
            </div>
          ))}
        </div>
